...
function RigidBody.prototype.update(self, dt)
    ...
    if self.velocity.y < 0 then
        ...
        ____self_velocity_8[____y_9] = ____self_velocity_8[____y_9] + self.drag.y * dt
        if self.velocity.y > 0 then
            self.velocity.y = 0
        end
    elseif self.velocity.y > 0 then
        local ____self_velocity_10, ____y_11 = self.velocity, "y"
        ____self_velocity_10[____y_11] = ____self_velocity_10[____y_11] - self.drag.y * dt
        if self.velocity.y < 0 then
            self.velocity.y = 0
        end
    end
